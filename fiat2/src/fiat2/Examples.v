...
Section Examples.
  ...
  Section WithMap.
    ...
    lazymatch goal with
    | H : type_of _ _ ?e (TList ?t), E : interp_expr _ _ ?e = _ |- type_of_value _ ?t => eapply type_sound in H; eauto;
                                                                                             rewrite E in H; inversion H; subst
    | H : type_of _ _ ?e ?t |- type_of_value (interp_expr _ _ ?e) ?t => eapply type_sound in H; eauto
    end.

    Local Ltac apply_Forall_In_vlist :=
        lazymatch goal with
        | H : Forall (fun v => type_of_value v ?t) _ |- _ => eapply List.Forall_In in H; eauto
        end.
    ...
    Definition preserve_ty (f : expr -> expr) :=
      forall Gstore Genv e t, type_of Gstore Genv e t -> type_of Gstore Genv (f e) t.
    ...
    Proof.
      ...
      injection H13 as H13; subst.
      constructor; auto.
    Qed.

    Lemma to_filter_head_preserve_sem : preserve_sem to_filter_head.
    Proof.
      unfold preserve_sem. intros Gstore Genv e t store env H H_str H_env.
      destruct e; auto. destruct e2; auto. destruct e2_2; auto.
    ...
    Proof.
      ...
      - inversion H1; subst. constructor. clear H1. induction l; auto.
        simpl in H; rewrite Bool.orb_false_iff in *. constructor.
        + inversion H3. inversion H0; subst. split; apply H8;
            destruct a; rewrite Bool.orb_false_iff in *; intuition.
        + inversion H0; inversion H3. apply IHl; intuition.
      - inversion H0; subst. econstructor.
        + apply IHe1; eauto; intuition.
    ...
    Proof.
      intros store env Gstore Genv tbl1 tbl2 p r x y t1 t2 t3 t t' H_tbl1 H_tbl2 H_free H_p H_str H_env.
      simpl. eapply type_sound in H_tbl1; eauto. inversion H_tbl1; subst.
      ...
      - inversion H_tbl2; subst.
        ...
        + rewrite Forall_forall in *; repeat apply well_formed_locals_step; intuition.
      - rewrite Forall_forall in *; repeat apply well_formed_locals_step; intuition.
    Qed.
    ...
    Proof.
      ...
      inversion H; subst. inversion H7; subst. inversion H9; subst. inversion H11; subst.
      rewrite Bool.orb_false_iff in *.
      eapply to_join_head_preserve_sem'; eauto; intuition.
    Qed.
    ...
    Proof.
      ...
      inversion H; subst.
      eapply type_sound in H9, H10; eauto. rewrite E1 in H9. rewrite E2 in H10.
      ...
      destruct (interp_expr store (map.put env x a) e3_1) eqn:E3_1;
        try (clear E2 H10; induction l0; auto; simpl;
             eapply not_free_immut_put_sem in E; rewrite <- E; rewrite E3_1; simpl;
             apply IHl0; inversion H7; intuition).
      destruct b.
      - clear E2 H10; induction l0; inversion H7; auto; simpl.
        eapply not_free_immut_put_sem in E; rewrite <- E; rewrite E3_1; simpl.
    ...
    Local Ltac apply_fold_expr_IH' e :=
      lazymatch goal with
      | IH : context [interp_expr _ _ e = interp_expr _ _ (fold_expr _ e)] |- _ => erewrite <- IH; clear IH; eauto
      end.
    ...
    Proof.
      ...
      revert Gstore Genv. induction e using expr_IH;
        intros; simpl; auto;
        apply H; invert_type_of; subst; econstructor; eauto.
      - rewrite fst_map_fst; auto.
      - clear H1 H5 H6 H7. generalize dependent tl.
        induction l; intros; destruct tl; try discriminate.
        + constructor.
        + simpl. constructor.
          * inversion H0; subst. inversion H4; subst. apply H5 in H8.
            destruct a; simpl in *. auto.
          * simpl in *; inversion H0; inversion H3; inversion H4; apply IHl; auto.
      - rewrite fst_map_fst; auto.
      - induction l; auto. constructor.
        + destruct a; inversion H0; inversion H3; intuition.
        + inversion H0; inversion H3; apply IHl; auto.
    ...
    Proof.
      ...
      induction e using expr_IH;
        intros; simpl;
        try now (erewrite <- H_sem; eauto).
      - invert_type_of. erewrite <- H_sem; simpl; repeat apply_fold_expr_IH; eauto.
        econstructor; eauto; apply fold_expr_preserve_ty; auto.
      - invert_type_of. subst. erewrite <- H_sem; simpl; repeat apply_fold_expr_IH; eauto.
      ...
      - invert_type_of. erewrite <- H_sem; simpl; repeat apply_fold_expr_IH; eauto;
        ...
        apply well_formed_locals_step; auto. eapply type_sound; eauto.
      - invert_type_of. erewrite <- H_sem; simpl; repeat apply_fold_expr_IH; eauto;
          try (econstructor; apply fold_expr_preserve_ty; eauto).
        destruct_one_match; auto.
        f_equal. apply In_flat_map_ext; intros. repeat apply_fold_expr_IH; eauto.
        apply well_formed_locals_step; auto. apply_type_sound. apply_Forall_In_vlist.
      - invert_type_of. erewrite <- H_sem; simpl; repeat apply_fold_expr_IH; eauto;
          try (econstructor; apply fold_expr_preserve_ty; eauto).
        destruct_one_match; auto. apply_fold_expr_IH' e2; eauto.
        apply In_fold_right_ext with (P := fun v => type_of_value v t); intros.
        + apply_type_sound.
        + apply_fold_expr_IH; eauto; intuition.
          * apply_type_sound. repeat apply well_formed_locals_step; auto.
            apply_type_sound; apply_Forall_In_vlist.
          * repeat apply well_formed_locals_step; auto. apply_type_sound; apply_Forall_In_vlist.
      - invert_type_of. erewrite <- H_sem; simpl; eauto.
        + do 2 f_equal. rewrite map_map; apply map_ext_Forall; rewrite Forall_forall.
          intros x H_in; destruct x. pose proof In_map_snd.
          match goal with
          | H : Forall2 _ _ _ |- _ => eapply Forall2_In_Permuted in H; eauto; do 3 destruct H5
          end.
          intuition; f_equal.
          rewrite Forall_forall in *. eapply H in H_in; simpl in *; eauto.
        + econstructor; eauto; try (rewrite fst_map_fst; auto).
          apply fold_expr_preserve_ty in H_ty.
          clear H0 H6 H7. generalize dependent tl. induction l; destruct tl; try discriminate; intuition.
          simpl. inversion H5; subst. constructor.
          * destruct a. simpl. apply H_ty. auto.
          * inversion H4; inversion H; apply IHl; auto.
      - invert_type_of. erewrite <- H_sem; simpl; eauto; try apply_fold_expr_IH r.
        econstructor; eauto. apply fold_expr_preserve_ty; auto.
      - invert_type_of. erewrite <- H_sem; simpl; eauto.
        + do 2 f_equal. rewrite map_map; apply map_ext_Forall; rewrite Forall_forall.
          intros x H_in; destruct x eqn:E.
          apply Forall_and_inv in H. intuition. rewrite Forall_forall in *.
          f_equal; [ pose proof (H3 x) | pose proof (H5 x) ]; subst; simpl in *; eapply H; eauto;
            apply H4 in H_in; simpl in *; apply H_in.
        + constructor. rewrite Forall_forall. intros p H_in.
          apply in_map_iff in H_in as [[k v] [H_p_p0 H_in]]; subst.
          rewrite Forall_forall in *. apply H4 in H_in as [H4L H4R].
          eapply fold_expr_preserve_ty in H4L, H4R; eauto.
      - invert_type_of. erewrite <- H_sem; simpl; eauto; try apply_fold_expr_IH.
      ...
      - invert_type_of. erewrite <- H_sem; simpl; eauto; try apply_fold_expr_IH.
        + destruct_one_match; auto. f_equal.
          apply In_filter_ext. intros x0 H_in. apply_fold_expr_IH.
          apply well_formed_locals_step; auto. apply_type_sound; apply_Forall_In_vlist.
        + constructor; apply fold_expr_preserve_ty; eauto.
      - invert_type_of. erewrite <- H_sem; simpl; eauto; try apply_fold_expr_IH.
        + destruct_one_match; auto. apply_fold_expr_IH.
          ...
          * rewrite Forall_forall. intros x0 H_in_x0. rewrite filter_In in H_in_x0; intuition.
            apply_fold_expr_IH. repeat apply well_formed_locals_step; auto;
              apply_type_sound; apply_Forall_In_vlist.
          * intros x0 H_in_x0. simpl. apply_fold_expr_IH.
            repeat apply well_formed_locals_step; auto;
              apply_type_sound; apply_Forall_In_vlist.
        + econstructor; apply fold_expr_preserve_ty; eauto.
      - invert_type_of. erewrite <- H_sem; simpl; eauto; try apply_fold_expr_IH.
        + destruct_one_match; auto. f_equal. apply map_ext_Forall; rewrite Forall_forall.
          intros x0 H_in. apply_fold_expr_IH. apply well_formed_locals_step; auto.
          apply_type_sound; apply_Forall_In_vlist.
        + econstructor; apply fold_expr_preserve_ty; eauto.
    Qed.
    ...
    Proof.
      ...
      - apply fold_expr_preserve_ty; [apply filter_pushdown_head_preserve_ty |].
        apply fold_expr_preserve_ty; [apply to_join_head_preserve_ty | auto].
      - erewrite fold_expr_preserve_sem; eauto.
        + erewrite fold_expr_preserve_sem; eauto.
