...
Section WithWord.
  ...
  Section WithMap.
    ...
    Goal interp_command map.empty map.empty (isSquare_elaborated 0)
    ...
    Lemma small_loop_false : forall x n, (S x * S x = n)%nat ->
                                          fold_left
                                            (fun (store' : locals) (v : Z) =>
                                               if
                                                 (match
                                                     map.get (map.put (map.put (@map.empty string (@sigT type (fun t : type => interp_type t)) locals) "n" (existT interp_type Int (Z.of_nat n))) "x" (existT interp_type Int v)) "x"
                                                   with
                                                   ...
                                                   | None => 0
                                                    ...
                                                    match
                                                      map.get (map.put (map.put (@map.empty string (@sigT type (fun t : type => interp_type t)) locals) "n" (existT interp_type Int (Z.of_nat n))) "x" (existT interp_type Int v)) "n"
                                                    with
                                                    | Some v0 => proj_expected Int v0
                                                    | None => 0
                                                    end)%Z
                                               then map.put store' "ans" (existT interp_type Bool true)
                                               else store') (eval_range 0 (S x)) map.empty
                                          = map.empty.
    Proof.
        ...
        fold_left
          (fun (store' : locals) (v : Z) =>
             if
               (match
                   map.get
                     (map.put (map.put map.empty "n" (existT interp_type Int (Z.pos (Pos.of_nat (S x * S x))))) "x" (existT interp_type Int v))
                     "x"
                 with
                 | Some v0 => proj_expected Int v0
                 | None => 0
             ...
             then map.put store' "ans" (existT interp_type Bool true)
             else store') (eval_range (Z.pos (Pos.of_succ_nat x + 1)) (S k)) (map.put map.empty "ans" (existT interp_type Bool true)) =
          map.put map.empty "ans" (existT interp_type Bool true).
    Proof.
      intros. induction k.
      - simpl eval_range.
    ...
    Theorem is_square_exists : forall (n : nat),
        ...
        interp_command map.empty map.empty (isSquare_elaborated (Z.of_nat n))
        = (map.put map.empty "ans" (existT interp_type Bool true)).
    Proof.
      intros n [x xSq].
