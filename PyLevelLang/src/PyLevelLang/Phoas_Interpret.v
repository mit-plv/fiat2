...
Section WithMap.
  ...
  Context {tstore : map.map string type} {tstore_ok : map.ok tenv}.

  Definition phoasify_envs_consistent (G : tenv) (l : locals) (env : locals) (store : locals) : Prop :=
    forall x,
      match map.get G x, map.get l x with
      | Some (t, is_mut), Some v =>
          let m := if is_mut then store else env in
          map.get m x = Some v /\ projT1 v = t
      | Some _, None => False
      | None, _ => True
      end.
  ...
  Proof.
    ...
    - repeat rewrite map.get_put_diff by assumption. pose proof (H x0) as H0.
      destruct (map.get G x0) eqn:E; auto. destruct p as [t' b]. destruct b.
      + exact H0.
      + rewrite map.get_put_diff by assumption. exact H0.
  Qed.
  ...
  Proof.
    induction e; intros e0 C H_equiv G H_wf store l used H_consistent; simpl;
      inversion H_equiv; subst; repeat eq_projT2_eq; simpl.
    ...
    - inversion H_wf. subst. destruct H_consistent as [_ HR]. unfold get_local.
      ...
      + exfalso; assumption.
    - inversion H_equiv; inversion H_wf. repeat eq_projT2_eq. f_equal. eauto.
    - inversion H_equiv; inversion H_wf. repeat eq_projT2_eq. simpl. f_equal; eauto.
    - inversion H_equiv; inversion H_wf. repeat eq_projT2_eq. f_equal.
    ...
    - inversion H_equiv; inversion H_wf. repeat eq_projT2_eq. f_equal.
      ...
      + eauto.
      + eauto.
    - inversion H_equiv; inversion H_wf. repeat eq_projT2_eq.
  ...
  Theorem phoasify_sound : forall {t : type} (e : expr t) (G : tenv),
      ...
      forall l store : locals,
        ...
        interp_expr l e = interp_phoas_expr store (phoasify map.empty e).
  Abort.
  ...
  Definition dephoasify_envs_consistent_closed (G : tenv_mut) (store l : locals) (used : list string) : Prop :=
    forall s : string, match map.get G s with
                       | Some t => In s used /\
                                     match map.get store s with
                                     | Some p => projT1 p = t /\ match map.get l s with
                                                                | Some p' => p = p'
                                                                | None => False
                                                                end
                                     | None => False
  ...
  Theorem dephoasify_sound : forall {t : type} (e : Phoas_expr t) (G : tenv_mut) ,
      ...
      forall (store l : locals) (used : list string),
        ...
        interp_phoas_expr store (e interp_type) = interp_expr l (dephoasify used (e const_string)).
Abort.
    (* =============== Wip =============== *)
